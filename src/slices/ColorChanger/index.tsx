...
import { Scene } from "@/slices/ColorChanger/Scene";
...
type KeycapTexture = (typeof KEYCAP_TEXTTURES)[number];

const ColorChanger: FC<ColorChangerProps> = ({ slice }) => {
  const [selectedTextureId, setSelectedTextureId] = useState(
    KEYCAP_TEXTTURES[0].id,
  );
  const [backgroundText, setBackroundText] = useState(KEYCAP_TEXTTURES[0].name);
  ...
  function handleTextureSelect(texture: KeycapTexture) {
    ...
    setBackroundText(
      KEYCAP_TEXTTURES.find((t) => t.id === texture.id)?.name || "",
    );
  }
  ...
  // console.log(handleTextureSelect);
  return (
    <section
    ...
    >
      ...
      {/* canvas */}
      <Canvas
        camera={{ position: [0, 0.5, 0.45], fov: 45, zoom: 1.5 }}
      ...
      >
        <div className="max-w-md shrink-0">
          <h2 className="font-bold-slanted mb-1 text-4xl uppercase lg:mb-2 lg:text-6xl">
            <PrismicRichText field={slice.primary.heading2} />
          </h2>
        ...
        <ul className="grid grow grid-cols-2 gap-3 rounded-2xl bg-white p-4 text-black shadow-lg sm:grid-cols-3 md:grid-cols-6 lg:grid-cols-3 xl:grid-cols-6">
          {KEYCAP_TEXTTURES.map((texture) => (
            <li key={texture.id}>
              <button
                onClick={() => handleTextureSelect(texture)}
                disabled={isAnimating}
                className={clsx(
                  "flex aspect-square flex-col items-center justify-center rounded-lg border-2 p-4 ease-out hover:scale-105 motion-safe:transition-all motion-safe:duration-300",
