...
const PurchaseButton: FC<PurchaseButtonProps> = ({ slice }) => {
  ...
  const handlePurchaseClick = async () => {
    setIsPressed(true);
    //TODO: add checkout locgic later
  ...
  useGSAP(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      const newWgth = 700 + normalizedX * 3000; // 700= lighter,1000=bolder
    ...
    gsap.set(textRef.current, { "--wdth": 85, "--wgth": 850 });
    ...
    >
      ...
      >
        ...
        <button
          ...
          disabled={isPressed}
          className={clsx(
            "group relative w-full overflow-hidden rounded-full border-5 border-gray-900 bg-linear-to-r/oklch from-sky-300 to-sky-600 px-6 py-6 ease-out focus:ring-[24px] focus:ring-sky-500/50 focus:outline-none motion-safe:transition-all motion-safe:duration-300 md:border-[12px] md:px-20 md:py-16",
        ...
        >
          <div className="absolute -translate-x-full bg-linear-to-r from-transparent via-white/40 to-transparent ease-out group-hover:translate-x-full motion-safe:transition-transform motion-safe:duration-1000" />

          <div className="relative z-10 flex items-center justify-center gap-6 md:gap-8">
            <span
              ref={textRef}
              style={{ "--wdth": 85, "--wgth": 850 } as React.CSSProperties}
            ...
            >
              {isPressed ? (
                <span className="flex items-center gap-4 md:gap-6">
                  <LuLoader className="size-12 animate-spin text-gray-950 md:size-16" />
                  Loading...
                </span>
              ) : (
